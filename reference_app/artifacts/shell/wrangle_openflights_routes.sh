...
python wrangle.py transform_blob \
    ...
    --blobname openflights__routes.json \
    --filename tmp/openflights/openflights__routes.json \
    --outfile  tmp/openflights/openflights__routes__wrangled.json \
    --out-container travel-routes-adf $1 $2 $3 

echo ''
echo 'first line of input file: tmp/openflights/openflights__routes.json'
head -1 tmp/openflights/openflights__routes.json

echo ''
echo 'first line of output file: tmp/openflights/openflights__routes__wrangled.json'
head -1 tmp/openflights/openflights__routes__wrangled.json

if [[ $M2C_COSMOS_LOAD_METHOD == "mongoimport" ]];
then
    echo ''
    echo 'executing mongoimport to db: travel coll: routes ...' 

    mongoimport \
        --uri $M2C_COSMOS_MONGO_CONN_STRING \
        --db travel \
        --collection routes \
        --file tmp/openflights/openflights__routes__wrangled.json \
...
then
    echo ''
    echo 'executing dotnet_mongo_loader to db: travel coll: routes ...' 

    dotnet run --project dotnet_mongo_loader/dotnet_mongo_loader.csproj \
        travel routes tmp/openflights/openflights__routes__wrangled.json \
...
then
    ...
    rm tmp/openflights/openflights__routes.json
