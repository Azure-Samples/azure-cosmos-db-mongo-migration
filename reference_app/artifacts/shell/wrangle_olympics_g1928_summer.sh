...
python wrangle.py transform_blob \
    --db olympics \
    --source-coll  g1928_summer \
    --in-container olympics-raw \
    --blobname olympics__g1928_summer.json \
    --filename tmp/olympics/olympics__g1928_summer.json \
    --outfile  tmp/olympics/olympics__g1928_summer__wrangled.json \
    --out-container olympics-games-adf $1 $2 $3 

echo ''
echo 'first line of input file: tmp/olympics/olympics__g1928_summer.json'
head -1 tmp/olympics/olympics__g1928_summer.json

echo ''
echo 'first line of output file: tmp/olympics/olympics__g1928_summer__wrangled.json'
...
then
    ...
    mongoimport \
        ...
        --db olympics \
        --collection games \
        --file tmp/olympics/olympics__g1928_summer__wrangled.json \
...
then
    ...
    dotnet run --project dotnet_mongo_loader/dotnet_mongo_loader.csproj \
        olympics games tmp/olympics/olympics__g1928_summer__wrangled.json \
...
then
    ...
    rm tmp/olympics/olympics__g1928_summer.json
